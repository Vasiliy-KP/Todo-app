...
const API_URL = "https://68ef4ef1b06cc802829cdb70.mockapi.io/api/v1/todos";
...
export const useTodoManagement = () => {
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      ...
      try {
        ...
        console.error("Dounloading data error:", error);
      }
    };
    loadInitialData();
  }, []);
  ...
  const onAdd = async (text, deadline) => {
    ...
    const updatedTodos = [...todos, newTodo]; //const name is the same but meaning is different then in the other function
  ...
  const handleUpdate = async (id, newText, newDeadline) => {
    ...
    try {
      ...
      console.error("Updeting error:", error);
      setTodos(todos);
    }
  };
  ...
  const confirmDeleteCompleted = async () => {
    ...
    for (const id of completedIds) {
      try {
        ...
        console.error(`Deleting task's error: ${id}`, error);
        failedIds.push(id);
      }
    }
